...
class Mosaic(object):
    ...
    def __init__(self, degrees=0, translate=.1, scale=.1, shear=10, border=2,perspective=0.0):
    ...
    def random_affine(self,img, targets):
        degrees, translate, scale, shear, border, perspective = self.degrees,self.translate,self.scale,self.shear,self.border,self.perspective
        border = -img.shape[0] // 2 // border
    ...
    def load_mosaic(self, imgs,ori_boxes,ori_labels):
        labels4 = []
        ...
        for i, img in enumerate(imgs):
            ...
            # Labels
            # print(f"labels={ori_labels[i].shape}, boxes={ori_boxes[i].shape}")
            x = np.concatenate([np.expand_dims(ori_labels[i],-1),ori_boxes[i]],-1)
            labels = x.copy()
    ...
    def __call__(self, results):
        """Call function to mosaic images.
        ...
        # labels and boxes will concated in load_mosaic
        img, label = self.load_mosaic(imgs,boxes.copy(),labels.copy())
        img, label = self.random_affine(img.copy(),label.copy())
        
        labels = np.int64(label[:,0]).copy()
        boxes = np.float32(label[:,1:]).copy()
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(degrees={self.degrees}, translate={self.translate}, scale={self.scale}, shear={self.shear}, border={self.border}, perspective={self.perspective})'
        return repr_str